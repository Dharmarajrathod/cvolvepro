...
export default function Home() {
  ...
  async function search(e: FormEvent) {
    ...
    try {
      ...
      if (typeof body.credits_remaining === "number") {
        const updated = updateAuthUserCredits(body.credits_remaining);
  ...
  const initials = (user?.name || user?.email || "CP").split(/\s+/).map(part => part[0]).join("").slice(0, 2).toUpperCase();
  ...
  return <main>
    <nav className="nav shell">
      ...
      {user && <div className="profile-wrap">
        ...
        <button className="profile" aria-label="Open profile" onClick={()=>setProfileOpen(open=>!open)}>{initials}</button>
    ...
    <section className="hero shell" id="search">
      ...
      <p className="lede">{user ? "Search public job boards and company career pages in one place. CvolvePro finds, compares, and ranks the roles that fit you." : "Login or register once to search roles, view job details, and check ATS scores without signing in again."}</p>
      ...
      <form className="search-card" onSubmit={search}>
        <div className="search-row">
          ...
          <button className="search-button" disabled={loading}>{loading ? <span className="spinner"/> : user ? <ArrowUpRight size={23}/> : <LogIn size={21}/>}<span>{loading ? "Searching" : user ? "Search jobs" : "Login to search"}</span></button>
        </div>
    ...
    <AnimatePresence mode="wait">
      ...
      {error && !loading && <motion.div className="notice shell" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}><X size={20}/><div><b>We couldn’t run that search</b><p>{error}</p></div>{error.toLowerCase().includes("choose a plan") && <button className="notice-action" onClick={()=>router.push("/plans")}>Choose plan</button>}</motion.div>}
    ...
    <footer className="shell"><a className="brand logo-brand" href="/jobs"><img src="/images/cvolvepro-logo.png" alt="Cvolve Pro"/></a><p>Move toward work that matters.</p><span>© 2026 CvolvePro</span></footer>
  </main>;
}
